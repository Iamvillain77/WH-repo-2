...
async def gen_command(client, message):
    try:
        ...
        if args[0] == "balance":
            if len(args) != 2:
                await message.reply_text("Usage: /gen balance <amount>")
                return
            ...
            # Store redeem code in the database with amount
            ...
            if len(args) != 2:
                await message.reply_text("Usage: /gen character <id>")
                return
...
async def redeem_command(client, message):
    try:
        ...
        # Check if redeem code exists and is not already used
        code_data = await db.redeem_codes.find_one({'code': redeem_code, 'used': False})
        ...
        if code_data:
            if 'amount' in code_data:
                ...
                await message.reply_photo(photo=img_url, caption=caption)
                await message.reply_text("Character redeemed successfully!")
